...
export class GlobalGenericValidator {
  ...
  public createErrorMessage(container: FormGroup, isFomSubmitted?: boolean): { [key: string]: string } {
    const errorMessage = {};
    ...
    for (const controlName in container.controls) {
      if (container.controls.hasOwnProperty(controlName)) {
        ...
        if (this.validatorMessage[controlName]) {
          ...
          if ((selectedControl.dirty || selectedControl.touched || isFomSubmitted) && selectedControl.errors) {
